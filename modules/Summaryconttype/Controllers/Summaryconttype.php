...
namespace Modules\Summaryconttype\Controllers;

use PhpOffice\PhpSpreadsheet\Spreadsheet;
use PhpOffice\PhpSpreadsheet\Writer\Xlsx;
use PhpOffice\PhpSpreadsheet\Style\Border;

class Summaryconttype extends \CodeIgniter\Controller
{
	private $client;
	...
	public function getAllData($prcode, $date_from, $hour_from, $hour_to)
	{
		$query_params = [
			'prcode' => $prcode,
			'date_from' => $date_from,
	...
	public function reportPdf($prcode = '', $date_from = '', $hour_from = '', $hour_to = '')
	{
		$mpdf = new \Mpdf\Mpdf();
		$result =   $this->getAllData($prcode, $date_from, $hour_from, $hour_to);

		$html = '';
		$html .= '
		<html>
			<head>
				<title>Summary Container Type</title>
		...
		$html .= '
			<table class="tbl_head_prain" width="100%">
				<tbody>
					...
					<tr>
					<td>As per Date : '. date('d/m/Y', strtotime($date_from)) .'</td>
					</tr>
					<tr>
					<td class="t-right">Container Operator : ' . $prcode . '</td>
					</tr>
				</tbody>
			</table>
		';
		...
		$mpdf->setHeader('|<h3>PT. CONTINDO RAYA<br>
					SUMMARY CONTAINER TYPE REPORT</h3>|PAGE {PAGENO}');
	...
	public function reportExcel($prcode = '', $date_from = '', $hour_from = '', $hour_to = '')
	{
		$exl = new Spreadsheet();
		$result =   $this->getAllData($prcode, $date_from, $hour_from, $hour_to);
