...
// "praid":42,"cpiorderno":"PI000D100000024","cpopr":"CMA","cpcust":"MIF","cpidish":"BEN","cpidisdat":"2021-09-01","liftoffcharge":0,"cpdepo":"11A25","cpipratgl":"2021-09-13","cpirefin":"","cpijam":"","cpivoyid":123,"cpives":"ARMADA SEJATI","cpicargo":"","cpideliver":"","cpilunas":0,"voyages":{"voyid":123,"vesid":"SINAR BATAM","voyno":"V246"},"vessels":{"vesid":"ARMADA SEJATI","vesopr":"SPIL","cncode":"ID","vestitle":"ARMADA SEJATI"
	...
	$('#printProforma').on("click", function(e){
		e.preventDefault();
		var praid = $(this).data("praid");
        window.open("<?php echo site_url('prain/print_proforma/'); ?>" + praid, '_blank', 'height=600,width=900,toolbar=no,directories=no,status=no, menubar=no,scrollbars=no,resizable=no ,modal=yes');
	});
	...
	$('#fileXls').bind('change', function(e) {
		...
		if($.inArray(fileType, validFileTypes) > 0){
			...
			// console.log(file);
			$.ajax({
				url: "<?=site_url('prain/import_xls_pra');?>",
				type:"POST",
				data:fd,
				dataType:"JSON",
		        contentType: false,
	...
	$("#insertContainerFromFile").on("click", function(e){
		...
		var praid = $("#praid").val();
		$.ajax({
			url: "<?=site_url('prain/insertContainerFromFile/');?>" + praid,
			type:"POST",
	       ...
	       //  contentType: false,
			...
			success: function(res) {
				if(res.status=="Failled") {
					...
					window.location.href="<?=site_url('prain')?>";					
				}
			}
		});
	});
