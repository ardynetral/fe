...
$(document).ready(function() {
	...
	var table = $('#ctTable').DataTable({
        ...
        "ajax": $.fn.dataTable.pipeline( {
            url: '<?=site_url('trucker/list_data');?>',
            pages: 5  
        } )
        ,
	...
	$("#saveData").click(function(e){
		...
		$.ajax({
			url: "<?php echo site_url('trucker/add'); ?>",
			type: "POST",
			data: formData,
			dataType: 'json',
			success: function(json) {
				if(json.message == "success") {
					...
					window.location.href = "<?php echo site_url('trucker'); ?>";
				} else {
					Swal.fire({
					  icon: 'error',
					  title: "Error",
	...
	$("#updateData").click(function(e){
		...
		console.log(formData);
		$.ajax({
			url: "<?php echo site_url('trucker/edit/'); ?>"+$("#cucode").val(),
			type: "POST",
			data: formData,
			dataType: 'json',
			success: function(json) {
	...
	function delete_data(code) {
		$.ajax({
			url: "<?php echo site_url('trucker/delete/'); ?>"+code,
			type: "POST",
			dataType: 'json',
			success: function(json) {
				if(json.message == "success") {
