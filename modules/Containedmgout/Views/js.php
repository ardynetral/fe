...
$(document).ready(function() {
	...
	var table = $('#ctTable').DataTable({
        ...
        "ajax": $.fn.dataTable.pipeline( {
            url: '<?=site_url('containedmgout/list_data');?>',
            pages: 5  
        } )
        ,
	...
	$("#saveData").click(function(e){
		...
		formData += "&chtype=" + $("#chtype").val();
		formData += "&chfrom=" + $("#chfrom").val();
		formData += "&chto=" + $("#chto").val();
		formData += "&chnote=" + $("#chnote").val();

		$.ajax({
			url: "<?php echo site_url('containedmgout/add'); ?>",
			type: "POST",
			data: formData,
			dataType: 'json',
			success: function(json) {
				if(json.message == "success") {
					...
					window.location.href = "<?php echo site_url('vessel'); ?>";
				} else {
					Swal.fire({
					  icon: 'error',
					  title: "Error",
	...
	function delete_data(code) {
		$.ajax({
			url: "<?php echo site_url('containedmgout/delete/'); ?>"+code,
			type: "POST",
			dataType: 'json',
			success: function(json) {
				if(json.message == "success") {
					...
					window.location.href = "<?php echo site_url('containedmgout'); ?>";
				} else {
					Swal.fire({
					  icon: 'error',
					  title: "Error",
