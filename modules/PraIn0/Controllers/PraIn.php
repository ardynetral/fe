...
class PraIn extends \CodeIgniter\Controller
{
	...
	public function edit($id)
	{
		...
		if ($this->request->isAJAX()) 
		{
			...
			// echo var_dump(count($_FILES['files']['name']));die();
			// $form_params = [
			// 	'praid' => $_POST['praid'],
			// 	'cpiorderno' => $_POST['cpiorderno'],
			// 	'cpidish' => $_POST['cpidish'],
	...
	public function approve_order($id) 
	{
		...
		if ($this->request->isAJAX()) 
		{
			...
			if((isset($result['status']) && ($result['status']=="Failled")) || isset($result2['status']) && ($result2['status']=="Failled"))
			{
				$data['message'] = $result['message'];
				echo json_encode($data);die();				
			}			
