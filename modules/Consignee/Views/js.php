...
$(document).ready(function() {
	...
	var table = $('#ctTable').DataTable({
        ...
        "ajax": $.fn.dataTable.pipeline( {
            url: '<?=site_url('consignee/list_data');?>',
            pages: 5  
        } )
        ,
		...
		$.ajax({
			...
			success: function(json) {
				if(json.message == "success") {
					...
					window.location.href = "<?php echo site_url('consignee'); ?>";
				} else {
					Swal.fire({
					  icon: 'error',
					  title: "Error",
	...
	$("#updateData").click(function(e){
		...
		console.log(formData);
		$.ajax({
			url: "<?php echo site_url('consignee/edit/'); ?>"+$("#cucode").val(),
			type: "POST",
			data: formData,
			dataType: 'json',
			success: function(json) {
	...
	function delete_data(code) {
		$.ajax({
			url: "<?php echo site_url('consignee/delete/'); ?>"+code,
			type: "POST",
			dataType: 'json',
			success: function(json) {
				if(json.message == "success") {
